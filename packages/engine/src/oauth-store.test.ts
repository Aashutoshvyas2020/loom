...
const root = await mkdtemp(join(tmpdir(), "loom-oauth-test-"));
const oauthConfig = {
...
try {
  ...
  await testAuthorizationPageHeaders(join(root, "authorization-headers"));
...
async function testDatabaseConfiguration(stateDir: string): Promise<void> {
  ...
  try {
    ...
    const migrations = database.sqlite
      .prepare("select version, name from loom_schema_migrations order by version")
      .all();
    assert.deepEqual(migrations, [
      { version: 1, name: "workspace-state" },
      { version: 2, name: "oauth-state" },
      { version: 3, name: "oauth-refresh-families" },
      { version: 4, name: "remove-workspace-state" },
    ]);
  } finally {
    database.close();
  }
...
function testExpiredTokenCleanup(stateDir: string): void {
  ...
  store.saveTokenPair({
    ...
    accessToken: { clientId: client.client_id, scopes: ["loom"], expiresAt: expiredAt },
    refreshTokenHash: "expired-refresh-hash",
    refreshToken: { clientId: client.client_id, scopes: ["loom"], expiresAt: expiredAt },
  });
  store.close();
      ...
      store.saveTokenPair(
        {
          ...
          accessToken: { clientId: client.client_id, scopes: ["loom"], expiresAt },
          refreshTokenHash: "new-refresh-hash",
          refreshToken: { clientId: client.client_id, scopes: ["loom"], expiresAt },
        },
        "old-refresh-hash",
      ),
      true,
    );
    assert.ok(store.getRefreshToken("old-refresh-hash")?.consumedAt);
...
async function testProviderRestartRotationAndRevocation(stateDir: string): Promise<void> {
  ...
  try {
    ...
    await assert.rejects(
      secondProvider.exchangeRefreshToken(client, issued.refresh_token, ["loom"], mcpUrl),
      InvalidGrantError,
    );
    await assert.rejects(secondProvider.verifyAccessToken(refreshed.access_token), InvalidTokenError);
    await assert.rejects(
      secondProvider.exchangeRefreshToken(client, refreshed.refresh_token, ["loom"], mcpUrl),
      InvalidGrantError,
    );
  } finally {
...
async function testAuthorizationPageHeaders(stateDir: string): Promise<void> {
