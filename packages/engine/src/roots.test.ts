...
assert.equal(expandHomePath("~/personal/loom"), resolve(home, "personal", "loom"));
assert.equal(expandHomePath("~user/project"), "~user/project");
assert.equal(expandHomePath("$HOME/project"), "$HOME/project");

assert.equal(
  assertAllowedPath("~/personal/loom", [join(home, "personal")]),
  resolve(home, "personal", "loom"),
);

assert.equal(
  assertAllowedPath("~/personal/loom", ["~/personal"]),
  resolve(home, "personal", "loom"),
);
