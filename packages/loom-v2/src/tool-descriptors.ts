...
export const TOOL_DESCRIPTORS: readonly ToolDescriptor[] = [
  descriptor(
    "loom_terminal",
    "Terminal",
    "Start, poll, or cancel one bounded noninteractive process inside an owner-configured local root. Jobs have output caps, timeouts, cancellation, process-group cleanup, and explicit high-confidence dangerous-command blocking.",
    object({
      action: action("start", "poll", "cancel"),
      command: text("Command for start.", 16_384), cwd: path,
      jobId: text("Job identifier for poll or cancel.", 128),
  ...
  descriptor(
    "loom_read",
    "Read",
    "Read bounded UTF-8, explicit base64, or a visible PNG/JPEG/GIF/WebP from an owner-configured local root. An unchanged repeat within ten project tool calls returns only an unchanged notice. Path and symlink escapes are rejected.",
