...
export const TOOL_DESCRIPTORS: readonly ToolDescriptor[] = [
  ...
  descriptor(
    "loom_agents",
    "Agents",
    "Start, poll, message, cancel, inspect, or delete bounded local coding subagents through an owner-configured compatible model endpoint. Subagents can use Loom coding tools but cannot create children or call loom_agents.",
    object({
      action: action("status", "start", "poll", "message", "cancel", "list", "read", "delete"),
      task: text("Explicit coding task for a new child-free subagent.", 32_768),
      agentId: text("Agent identifier for poll, message, cancel, read, or delete.", 128),
      text: text("Continuation message for a retained agent.", 32_768),
      cwd: path,
      model: text("Optional provider model override.", 512),
      ...
      state: action("queued", "running", "waiting", "completed", "failed", "cancelled", "interrupted"),
  ...
  descriptor(
    "loom_read",
    "Read",
    "Read bounded UTF-8, explicit base64, or a model-visible PNG/JPEG/GIF/WebP from an owner-configured local root. Images are returned inline without file attachments. An unchanged repeat within ten project tool calls returns only an unchanged notice. Path and symlink escapes are rejected.",
    object({ path, offset: { type: "integer", minimum: 0 }, length: { type: "integer", minimum: 1, maximum: 10_485_760 }, encoding: { type: "string", enum: ["utf8", "base64"] } }, ["path"]),
    result,
    ["loom"],
  ...
  descriptor(
    "loom_memory",
    "Memory",
    "Read or curate Loom's global durable MEMORY.md. Add only verified reusable facts; replace or remove stale facts. Never store transient output, guesses, or secrets.",
    object({
      action: action("read", "add", "replace", "remove"),
      content: { ...text("Verified reusable fact to add.", 16_384), minLength: 1 },
      oldText: { ...text("Exact memory text to replace or remove.", 16_384), minLength: 1 },
      newText: text("Replacement memory text; empty removes the matched text.", 16_384),
    }, ["action"]),
    result,
    ["loom"],
    { readOnlyHint: false, openWorldHint: false, destructiveHint: true },
